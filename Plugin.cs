...
namespace Butcher
{
    ...
    {
        public override string ModuleName => "The Butcher Acheivement";
        ...
        public Dictionary<CCSPlayerController, PlayerKnifeKillCount> _playerKillCount { get; set; } = new();
        ...
        public void CountKill(CCSPlayerController client)
        {
            ...
            if (_playerKillCount[client!].KillCount >= Config.MaxKnifeKillCount)
            {
                var steamid = client.AuthorizedSteamID?.SteamId64;
        ...
        public async Task TaskComplete(CCSPlayerController client, ulong steamid)
        {
            ...
            if (response != null)
            {
                ...
                Server.NextFrame(() =>
                {
                    client.PrintToChat($" {ChatColors.Green}[Acheivement]{ChatColors.Default} You acheive 'The Butcher' (Kill enemies with knife for {Config.MaxKnifeKillCount} times)");
